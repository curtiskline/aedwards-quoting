...
4. **`deploy_web.sh` is safe on staging as of task 449** (see `deploy/README.md`). The old footguns — defaulting `EMAIL_DELIVERY_ENABLED=true` and reverting the host `DATABASE_URL` to sqlite (lessons 21/22) — are fixed structurally: the host `.env` is authoritative (`deploy/merge_env.sh`), delivery can never be enabled by a deploy, and a missing `DATABASE_URL` hard-fails instead of falling back to sqlite. Preview any deploy first with `--dry-run`. Remaining caveat: a full deploy replaces the app source tree, so it would erase the manual hotpatch noted in finding 3 — deploy from a branch that already contains those fixes.
